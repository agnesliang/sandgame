...
def is_move_ok(grid, x_to, y_to):
    ...
    False
    """
    if not grid.in_bounds(x_to, y_to) or grid.get (x_to, y_to) != None :
       return False
    return True
...
def move_water(grid, x, y):
    ...
    [['w', 'w', 'w'], ['r', None, None]]
    """
    if is_move_ok(grid, x,  y + 1): #down
        grid.set(x, y, None)
        grid.set(x, y + 1, 'w')
        return grid
    if is_move_ok(grid, x - 1, y + 1): #down-left
        grid.set(x, y, None)
        grid.set(x - 1, y + 1, 'w')
        return grid
    if is_move_ok(grid, x + 1, y + 1): #down-right
        grid.set(x, y, None)
        grid.set(x + 1, y + 1, 'w')
        return grid
    grid.set(x, y, None)
