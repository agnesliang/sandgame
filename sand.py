...
def is_move_ok(grid, x_from, y_from, x_to, y_to):
    ...
    True
    >>> is_move_ok(grid, 1, 0, 2, 1)  # 1
    False
    >>> is_move_ok(grid, 1, 0, 1, 1)  # 2
    True
    >>> is_move_ok(grid, 1, 0, 0, 1)  # 3
    True
    >>> is_move_ok(grid, 2, 0, 0, 1)  # 3
    True
    """
    ...
    if grid.get (x_to,y_to) != None:
        return False
    if (x_from != x_to) and grid.get(x_to, y_from) != None:
        return False
    return True
...
def do_gravity(grid, x, y):
    ...
    """
    if grid.get(x,y) == 's':
        if is_move_ok(grid, x, y, x, y + 1): #down
            do_move(grid, x, y, x, y + 1)
            return do_move(grid, x, y, x, y + 1)
        if is_move_ok(grid, x, y, x - 1, y + 1): #down-left
            do_move(grid, x, y, x - 1, y + 1)
            return do_move(grid, x, y, x - 1, y + 1)
        if is_move_ok(grid, x, y, x + 1, y + 1): #down-right
            do_move(grid, x, y, x + 1, y + 1)
            return do_move(grid, x, y, x + 1, y + 1)
    return grid
...
def do_whole_grid(grid, brownian):
    ...
    (tests and code TBD)
    >>> grid = Grid.build([['s', None, 's'], ['s', None, 's']])
